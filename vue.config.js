...
module.exports = {
  ...
  chainWebpack: config => {
    ...
    config.resolve.alias.set('script', resolve('src/assets/script'))
    config.resolve.set('extensions', ['.js', '.ts', '.vue', '.json', '.less'])
  },
  // configureWebpack: () => {
  //   return {
  //     optimization: {
  ...
  // CSS 相关选项
  css: {
    // // 将组件内的 CSS 提取到一个单独的 CSS 文件 (只用在生产环境中) 这里用true将导致css文件热刷新失效
